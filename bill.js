...
router.post('/generateReport', auth.authenticateToken, (req, res) => {
    ...
    var query = "insert into bill(name,uuid,contactNumber,paymentMethod,total,productDetails,createdBy) values(?,?,?,?,?,?,?,?)";
    connection.query(query, [orderDetails.name, generatedUuid, orderDetails.email, orderDetails.contactNumber, orderDetails.paymentMethod, orderDetails.totalAmount, orderDetails.productDetails, res.locals.email], (err, results) => {
        if (!err) {
            ejs.renderFile(path.join(__dirname, '', report.ejs), { productDetails: productDetailsReport, name: orderDetail.name, email: orderDetails.email, contactNumber: orderDetails.contactNumber, paymentMethod: orderDetails.paymentMethod, totalAmount: orderDetails.totalAmount }, (err, results) => {
                if (!err) {
...
router.post('getpdf',auth.authenticateToken,function(req,res){
    ...
    if(fs.existsSync(pdfPath)){
        ...
        var productDetailsReport = JSON.parse(orderDetails.pr)
        ejs.renderFile(path.join(__dirname, '', report.ejs), { productDetails: productDetailsReport, name: orderDetail.name, email: orderDetails.email, contactNumber: orderDetails.contactNumber, paymentMethod: orderDetails.paymentMethod, totalAmount: orderDetails.totalAmount }, (err, results) => {
            if (!err) {
                return res.status(500).json(err);
            }
